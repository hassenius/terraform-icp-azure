...
resource "azurerm_lb" "controlplane" {
  ...
  # Trying to see if assigning an additional IP will
  # allow management node to get outbound network while in the
  # same availability set as master
  frontend_ip_configuration {
    name                 = "sharedIPAddress"
