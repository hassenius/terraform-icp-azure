...
resource "azurerm_public_ip" "master_pip" {
  name                         = "${var.cluster_name}-${random_id.clusterid.hex}-master"
  location                     = "${var.location}"
  resource_group_name          = "${azurerm_resource_group.icp.name}"
  sku                          = "Standard"
  public_ip_address_allocation = "Static"
  domain_name_label            = "${var.cluster_name}-${random_id.clusterid.hex}-master"
}

resource "azurerm_public_ip" "proxy_pip" {
  name                         = "${var.cluster_name}-${random_id.clusterid.hex}-proxy"
  location                     = "${var.location}"
  resource_group_name          = "${azurerm_resource_group.icp.name}"
  sku                          = "Standard"
  public_ip_address_allocation = "Static"
  domain_name_label            = "${var.cluster_name}-${random_id.clusterid.hex}-proxy"
}
